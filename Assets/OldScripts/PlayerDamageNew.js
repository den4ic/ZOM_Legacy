...
function OnGUI () {
	...
	GUI.Label (Rect(40, Screen.height - 80,90,90)," Health: ");
	GUI.Label (Rect(115, Screen.height - 50,60,60),"" +hitPoints.ToString("F0"), style1);
...
function rskill(quantia : int){
regenerationSkill = quantia;
}
function save(salve : boolean){
	if(salve){
		PlayerPrefs.SetInt("vida", hitPoints);
	}
}
function load(loaded : boolean){
	if(loaded){
		hitPoints = PlayerPrefs.GetInt("Vida", hitPoints);
	}
